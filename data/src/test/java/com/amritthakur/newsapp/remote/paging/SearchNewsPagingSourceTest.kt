...
class SearchNewsPagingSourceTest {
    ...
    private val pagingSource = SearchNewsPagingSource(mockRemoteDataSource, query)
    ...
    fun `load should return first page with correct keys when loading page 1`() = runTest {
        ...
        coEvery {
            mockRemoteDataSource.searchNews(
                query = query,
                pageSize = 20,
                page = 1
            )
    ...
    fun `load should return LoadResult Error when remote data source returns Result Error`() =
        runTest {
            ...
            assertEquals(
                "Code: 429, ErrorCode: rateLimited, ErrorMessage: Rate limit exceeded",
                exception.message
            )
        }
    ...
    fun `load should pass correct query and parameters to remote data source`() = runTest {
        ...
        val customPagingSource = SearchNewsPagingSource(mockRemoteDataSource, customQuery)
    ...
    fun `load should handle different query types correctly`() = runTest {
        ...
        val specialPagingSource = SearchNewsPagingSource(mockRemoteDataSource, specialQuery)
